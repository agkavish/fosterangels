...
<%
	if(uProfile.getTitle() != null && !uProfile.getTitle().equalsIgnoreCase("fa")){
 %>
...
<label class="Alerts" for="AlertSection"><a href="#" class="alert-link">Requests</a> / <a href="#" class="alert-link">Attach Receipts</a> / Request <%= cwRequest.getRequestNumber() %></label>

<div class="AlertSection">

<form action="<%= blobstoreService.createUploadUrl("/cwrequest/attach") %>" method="post" enctype="multipart/form-data" role="form" name='cpsfileupload' onSubmit="return true;" >

<input type="hidden" name="reqkey" value="<%=KeyFactory.keyToString(cwRequest.getKey())%>"/>

  <div class="form-group">
    <label for="fileAttach">File</label>
    <input type="file" class="form-control" name="fileAttach" id="fileAttach" multiple autofocus accept="image/*, application/pdf" />
  </div>
<div class="form-group">
  <button class="btn btn-lg btn-primary btn-block " type="submit">Upload Receipt</button>
  </div>
</form>
